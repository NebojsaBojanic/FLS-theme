<?php 
	$spaceBottom 		= get_sub_field('gallery_space_bottom');
?>
<div class="<?php if($spaceBottom){echo 'acf-block';} ?> acf-gallery gallery"><!-- start gallery -->
	...
	<?php if( $gallery ): ?>
		...
		<ul id="sync1" class="gallery-slide owl-carousel owl-theme">
			<?php
			foreach( $gallery as $image ): ?>
		...
		<ul id="sync2" class="gallery-thumbnails owl-carousel owl-theme">
			<?php
			foreach( $gallery as $image ): ?>
				<li>
					<img src="<?php echo $image['sizes']['img_sixteen_nine']; ?>" alt="<?php echo $image['alt']; ?>" title="<?php $image['title'] ?>">
